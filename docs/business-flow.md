...
WhatMint 不是单纯的 NFC 产品，而是一个自营的现实世界 UI App：让日常物品在关键生活场景中获得数字表达。当前实现优先服务“一物一场景一魔法”的实体入口闭环，典型样本是纸巾小狗和桌面秘境。

核心链路：

```text
认识 WhatMint -> 邀请/获得实体入口 -> 官方录入订单并生成 token -> token/NFC 链接写入物件 -> 创作中心按内容定义创建内容实例 -> 内容绑定到物件实例 -> 触碰物件打开对应数字表达 -> 接入 Mint Space
```
...
IP 定义，例如纸巾小狗、桌面秘境。包含实体信息、场景信息、创作者信息和官方设定。产品默认是一个 IP 绑定一个应用、一个实体入口和一个具体生活场景；技术上仍通过关系表保持可扩展。

### `application_definitions`

应用定义。用于声明体验流、内容模板、渲染方式、事件订阅和 Skill 能力。轻应用是 WhatMint 自营功能入口，不是当前对外开放的第三方平台。

关系：`ip_definitions *-* application_definitions`，当前业务上通常一对一。

### `content_definitions`

内容创作能力定义，例如纸巾小狗的“AR 召唤节点”或桌面秘境的“AR 场景节点”。创作中心按它生成引导。
...
- 纸巾小狗、桌面秘境这类新应用优先按核心对象、官方内容实例、内容定义和 OS 渲染协议组织体验。
- 官方内容通过创作中心创建，再由官方权限手动选择默认内容，避免形成第二套官方内容运营后台。
- 官方后台不再保留旧 `works/content_collections/app_bindings` CMS；内容资产只认 `content_instances/resources/content_definitions` 和实体默认内容关系。
