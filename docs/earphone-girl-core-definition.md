...
> 2026-07-20 更新：耳机小姐作为“插图 + 音频讲故事应用”的产品范式已降级，不再作为 WhatMint 首版强应用形态推进；但“观察者 / 旅行者 / 世界观串联者”的 IP 角色仍然保留。后续应把她从弱内容播放器转向世界观察者、关系发现者、数字空间旁白或首页级世界观入口。
...
- 原“故事空间”可以作为验证过的应用草案保留，但不作为首版强产品方向。
- 耳机小姐更适合承担 WhatMint 世界中的观察者角色：发现 IP 关系、讲述用户资产空间变化、解释世界状态。
- 如果后续继续使用耳机小姐，应优先围绕“世界观察 / 关系雷达 / 今日观察 / 资产空间旁白”设计，而不是围绕多页插图音频播放器设计。
...
- 作为 WhatMint 世界里的观察者和旅行者
- 作为首页或资产数字空间中的世界观旁白
- 发现不同 IP 之间的官方设定关系和用户资产状态变化
- 把“这些物件生活在同一个世界”这件事表达出来
- 为未来 AI 微智能提供自然的人格入口，但不在首版强行依赖 AI
...
如果按这份定义落表，耳机小姐足够说明 WhatMint 如何把一个 IP 映射到核心表：
...
但这不再意味着“插图 + 音频故事空间”是首版最优应用形态。

更准确的结论是：

- 这份定义验证了核心表可以承载一个有角色、有应用、有内容、有关系的 IP。
- 耳机小姐的角色价值高于故事播放器价值。
- 后续应把她作为观察者 IP 保留，把具体应用形态重新设计为数字空间观察、关系发现或世界入口。

如果后续继续推进她，优先做三件事：

1. 在首页或资产空间中验证“世界观察者”表达是否有产品力。
2. 在商城和 IP 详情页按 `ip_definitions + relation_links` 展示她和世界的关系。
3. 再决定是否需要保留、替换或重做她的 `application_definitions`。
