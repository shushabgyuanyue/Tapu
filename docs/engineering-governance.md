...
详细执行标准见：[development-standards.md](development-standards.md)。本文偏治理护栏，开发细则以该文档为准。
...
- `check:copy`：扫描非管理端 `src/views`、`src/components`、`src/composables`，阻止新的中文用户文案直接散落进页面逻辑。
...
- 如果当前需求会继续推高 500+ 行文件，必须在同一轮开发中主动拆分。
- 不允许把“大文件整理”默认推迟到后续治理轮次。
...
- 新的中文字符串如果确实不是用户文案，例如图片文件名、IP 规则匹配或受控 allowlist，需要在 `scripts/check-copy-governance.mjs` 中显式登记理由。
- 新应用是否复用内容资产、Mint Studio、上传、预览、绑定等公共能力。
- 新样式是否复用现有设计 token 或模块级样式，而不是引入孤立视觉语言。
- 新文件是否会让已有大文件继续膨胀；如果会，先拆再加。
- 新增接口和路由是否延续当前 `routePermissions + operation + contract` 范式，而不是临时绕开现有治理链路。
